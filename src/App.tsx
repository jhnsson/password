...
const App: React.FC = () => {
  ...
  const handleGeneratePassword = () => {
    ...
    for (let i = 0; i < passwordLength; i++) {
      ...
      passwordString += selectedPool.charAt(
    ...
    <div className="bg-[#2C3137] text-[#FFFEFE] fira-600 w-full min-h-screen overflow-hidden flex flex-col justify-center items-center whitespace-nowrap">
      ...
      <div>
        <section className="flex flex-row items-center gap-2 mb-4">
          <span onClick={handleGeneratePassword}>
            <img src={generateIcon} className="w-8 h-8 cursor-pointer" />
          </span>
          <span onClick={handleCopy}>
            <img src={copyIcon} className="w-8 h-8 cursor-pointer" />
          </span>
          <span className="ml-2 text-4xl">
            ...
            <input
              ...
              className="bg-[#888C90] text-[#075FE3] text-center font-bold"
            />
          </span>
        </section>
        <section className="grid grid-cols-2 gap-6">
          {Object.keys(options).map((option, index) => (
            <span
              key={index}
              onClick={() => handleSelection(option as keyof typeof options)}
              className="flex flex-row justify-center"
            >
              <p
                className={`${
                  !options[option as keyof typeof options]
                    ? "text-[#888C90] hover:text-white"
                    : "text-[#075FE3] hover:text-[#888C90]"
                } text-3xl font-bold cursor-pointer mx-auto`}
